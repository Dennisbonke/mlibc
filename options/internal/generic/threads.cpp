...
int thread_mutex_unlock(struct __mlibc_mutex *mutex) {
	...
	// Normal mutexes do not require the unlocking thread to be the owner. This
	// is also needed to unlock a mutex inherited across fork().
	if (flags & (__mlibc_mutex::mutexRecursive | __mlibc_mutex::mutexErrorCheck)) {
		auto state = __atomic_load_n(&mutex->__mlibc_state, __ATOMIC_RELAXED);
		if ((state & __mlibc_mutex::mutex_owner_mask) != this_tid)
			return EPERM;
	}

	// Decrement the recursion level and unlock if we hit zero. Normal and
	// error-checking mutexes cannot be recursively locked.
	if (flags & __mlibc_mutex::mutexRecursive) {
		__ensure(mutex->__mlibc_recursion);
